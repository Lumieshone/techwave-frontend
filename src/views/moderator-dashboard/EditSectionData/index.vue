      ...
      <v-col>
        ...
        >
          <template v-slot:[`item.sectionAvatar`]="{ item }">
            <v-avatar class="my-2"
              ><img alt="Avatar" :src="item.sectionAvatar"
            /></v-avatar>
          </template>
            ...
            <v-tooltip top class="mr-1">
              <template v-slot:activator="{ on, attrs }">
                <v-btn icon v-bind="attrs" v-on="on">
                  <v-icon @click="stepToSection(item)">
...
import { getSectionInfoById } from "@/api/moderator";
  ...
  methods: {
    ...
    stepToSection(item) {
      this.$router.push({
        ...
        params: { id: item.sectionId },
      });
    },
    stepToManage(item) {
      this.$router.push({
        path: "/moderator-dashboard/" + item.sectionId,
        params: { id: item.sectionId },
      });
    },
    getSectionInfo() {
      getSectionInfoById(this.$route.params.sectionId)
        .then((res) => {
          if (res.code === 20000) {
