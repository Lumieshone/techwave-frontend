  ...
  <v-card height="570" class="mx-4 my-5" :loading="this.loading">
    ...
    <v-row no-gutters justify="center">
      <v-card width="100%">
        <v-row>
          <v-col cols="10">
            <v-tabs light color="#6A5ACD" show-arrows>
              <v-tab
                v-for="(folder, index) in folders"
                :key="folder.id"
                @change="getCollects(folder.id, folder.folderName)"
              >
                <v-icon v-if="index === 0" class="mr-1"
                ...
                <template>
                  <v-list-item-content>
                    ...
                    <v-list-item-subtitle
                      v-text="item.posterName"
                    ></v-list-item-subtitle>
                  </v-list-item-content>
                  <v-list-item-action>
                    <v-list-item-action-text
                      v-text="item.sectionName"
                    ></v-list-item-action-text>
                  </v-list-item-action>
                  <v-list-item-action>
                    <v-list-item-action-text
